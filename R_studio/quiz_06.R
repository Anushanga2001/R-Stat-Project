...
# Test whether there is a significant difference in assembly task completion time significantly differs across lighting conditions.
# at 0.05 level of significance.

# H0: There is NO significant difference in assembly task completion time significantly differs across lighting conditions
# H1: There is significant difference in assembly task completion time significantly differs across lighting conditions

# Reading the LoadTimes.csv.
workers_efficiency <- read.csv("C:/Users/AnushangaKularachchi/Downloads/Q6.csv")

# Extracting load times of three browsers
D_L <- workers_efficiency$Dim_Light
N_L <- workers_efficiency$Normal_Light
B_L <- workers_efficiency$Bright_Light
...
# Conclusion: the mean worker efficiency among these 3 conditions follows a normal distribution 

## 2. Testing for equal variance using leveneTest
library(car)

# Combining production speeds of machines A, B, and C
efficiency_speed <- c(D_L, N_L, B_L)
efficiency <- rep(c("D", "N", "B"), each = 15)
data <- data.frame(efficiency_speed, efficiency)

# leveneTest
# H0: The efficiency speeds of three conditions have equal variance
leveneTest(efficiency_speed ~ efficiency, data)
# Conclusion: The efficiency speeds of three conditions have equal variance

## 3. The conditions speeds of three conditions are independent.

## Perform one-way ANOVA
# H0: There is NO significant difference between the mean efficiency speeds of three conditions
# H1: There is a significant difference between the mean efficiency speeds of three conditions

# We perform ANOVA parametric test as normality and equal variance conditions are satisfied.
res.aov <- aov(efficiency_speed ~ efficiency, data = data)
summary(res.aov)
# We can conclude that there is a significant difference between the mean efficiency speeds of 
# three conditions at 0.05 level of significance.

# Alternatively, we perform ANOVA non-parametric test although normality and equal variance conditions are satisfied.
kruskal.test(efficiency_speed ~ efficiency, data = data)
# We can conclude that there is a significant difference between the mean efficiency speeds of 
# three conditions at 0.05 level of significance.


## Now check the variance of the samples
